...
const APIKey = '646aedc72ef7dac5fba45e44'
const getUrl = currency => `https://v6.exchangerate-api.com/v6/${APIKey}/latest/${currency}`

const getErrorMessage = errorType => ({
  'unsupported-cod': 'Não temos a moeda em nosso banco',
  'malformed-reques': 'Sua solicitação não atende a estrutura a seguir: https://v6.exchangerate-api.com/v6/646aedc72ef7dac5fba45e44/latest/USD',
  'invalid-key': 'A chave da API não é válida',
  'inactive-account': 'Seu endereço de e-mail não foi confirmado',
  'quota-reached': 'Sua cota atingiu o n° máximo de solicitações'
...
const fetchExchangeRate = async url => {
  try {  
    ...
    if (!response.ok) {
      throw new Error('Sua conexão falhou. Impossível obter as informações')
    }
...
const getOptions = (selectedCurrency, conversion_rates) => {
  const setSelectedAtribbute = currency =>
    currency === selectedCurrency ? 'selected' : ' '
  const getOptionsAsArray = currency => 
    `<option ${setSelectedAtribbute(currency)}>${currency}</option>`
...
const getNotRoundedExchangeRate = conversion_rates => {
  const currencyTwo = conversion_rates[currencyTwoEl.value]
  return `1 USD = ${1 * currencyTwo} ${currencyTwoEl.value}`
}
